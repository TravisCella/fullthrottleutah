...
const CUSTOMER_ZONES = [
  {
    ...
    label: 'Bow / front',
    icon: '↗',
    hint: 'Stand in front, capture the full nose and hull',
  },
  {
    id: 'port',
    label: 'Port side (left)',
    icon: '←',
    hint: 'Full length from front to back, stand 6ft away',
  },
  {
    id: 'starboard',
    label: 'Starboard side (right)',
    icon: '→',
    hint: 'Full length from front to back, stand 6ft away',
  },
  {
    ...
    label: 'Stern / rear',
    icon: '↙',
    hint: 'Capture jet nozzle, rear bumper, and registration',
  },
  {
    id: 'deck',
    label: 'Deck / seat area',
    icon: '⬆',
    hint: 'Show seat condition, storage lids, handlebars',
  },
  {
    id: 'gauge',
    label: 'Gauge / display',
    icon: '◉',
    hint: 'Power on and capture the screen clearly',
  },
  {
    id: 'trailer',
    label: 'Trailer & lights',
    icon: '▬',
    hint: 'Hitch, bunks, and all trailer lights',
  },
  {
    id: 'extras',
    label: 'Gear check',
    icon: '⚓',
    hint: 'Life vests, anchor, safety flag, fuel level',
  },
];
...
const OWNER_ZONES = [
  ...
  {
    id: 'impeller',
    label: 'Impeller inspection',
    icon: '⚙',
    hint: 'Photograph impeller blades, wear ring, and intake grate. Note any nicks, dents, or debris damage.',
  },
  {
    id: 'hull-underside',
    label: 'Hull underside',
    icon: '▼',
    hint: 'Check for deep scratches, cracks, or gelcoat damage on the hull bottom',
  },
];
    ...
    <div>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 10,
          background: busy ? '#f0ede6' : 'transparent',
...
export default function InspectionV2() {
  ...
  const runCompare = () => {
    ...
    aiCompare(compareOut.trim(), compareIn.trim(), authPass, (result) => {
      if (!result) {
        setAiError(
          'AI analysis failed. Check the inspection IDs and try again, or check Vercel logs.'
        );
      } else {
        setAiResult(result);
      }
        ...
        <div style={{ width: '100%', maxWidth: 360, padding: '32px 24px' }}>
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 13, color: muted, marginTop: 6 }}>
              Staff access only. Enter admin password.
            </div>
          </div>
          <input
            type="password"
            ...
            style={{
              ...
              padding: '12px 14px',
              borderRadius: 10,
              border: `1.5px solid ${authError ? red : bdr}`,
              fontSize: 14,
              fontFamily: 'inherit',
          ...
          <button onClick={handleAuthSubmit} style={btn(accent, '#fff')}>
            Continue →
          </button>
        </div>
      </div>
        ...
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span
                style={{
                  ...
                  padding: '2px 6px',
                  borderRadius: 4,
                  background: isCustomer ? '#e8f4fd' : '#fef0ea',
          ...
          {showSelectButtons && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {isCustomer && (
                <button
                  onClick={() => onUseAsCheckout(insp.inspectionId)}
              ...
              {!isCustomer && (
                <button
                  onClick={() => onUseAsCheckin(insp.inspectionId)}
                  style={{ ...outline, padding: '5px 10px', fontSize: 11, whiteSpace: 'nowrap' }}
                >
                  Use as IN
                </button>
          ...
          {!showSelectButtons && (
            <button
              onClick={() => navigator.clipboard?.writeText(insp.inspectionId)}
      ...
      <div style={{ padding: '16px 16px 100px', maxWidth: 500, margin: '0 auto' }}>
        ...
        {!role && (
          <div style={{ marginTop: 24 }}>
            ...
            <div style={{ fontSize: 13, color: muted, marginBottom: 24 }}>
              Select your role to begin the inspection process.
            </div>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: muted, marginTop: 2 }}>
                  Photograph the watercraft before your rental
                </div>
              </div>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: muted, marginTop: 2 }}>
                  Check-in with impeller & hull inspection
                </div>
              </div>
              <span style={{ fontSize: 18, color: accent }}>→</span>
            </div>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: muted, marginTop: 2 }}>
                  Claude Vision compares check-out vs check-in photos
                </div>
              </div>
              <span style={{ fontSize: 18, color: accent }}>→</span>
            </div>
              ...
              >
                <div style={{ fontSize: 13, fontWeight: 600 }}>
                  ...
                  {recentInspections.length > 0
                    ? ` (${recentInspections.length} in 30 days)`
                    : ' (30 days)'}
                </div>
                <button
                  onClick={loadRecentInspections}
                  style={{ ...outline, padding: '4px 10px', fontSize: 11 }}
                >
                  {loadingRecent ? '...' : '↻'}
                </button>
              </div>
              {loadingRecent && recentInspections.length === 0 && (
                <div style={{ textAlign: 'center', padding: 16, color: muted, fontSize: 12 }}>
                  Loading...
                </div>
              )}
              {!loadingRecent && recentInspections.length === 0 && (
        ...
        {compareMode && (
          <div style={{ marginTop: 20 }}>
            <button
              onClick={reset}
              style={{ ...outline, padding: '6px 12px', fontSize: 12, marginBottom: 16 }}
            >
              ← Back
            </button>
            ...
            <div style={{ fontSize: 13, color: muted, marginBottom: 6 }}>
              Powered by Claude Sonnet 4.6 vision — analyzes the actual photos.
            </div>
            <div
              style={{
                fontSize: 11,
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              >
                Recent inspections{' '}
                {recentInspections.length > 0 ? `(${recentInspections.length})` : ''}
              </div>
              {recentInspections
                .filter((insp) => {
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 12, fontWeight: 500, display: 'block', marginBottom: 4 }}>
                Check-out ID (customer departure)
              </label>
              <input
                value={compareOut}
                onChange={(e) => setCompareOut(e.target.value)}
                placeholder="e.g. 1779070781015-s13hofo67"
                style={{
                  width: '100%',
                  padding: '10px 12px',
                  borderRadius: 8,
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, fontWeight: 500, display: 'block', marginBottom: 4 }}>
                Check-in ID (owner return)
              </label>
              <input
                value={compareIn}
                onChange={(e) => setCompareIn(e.target.value)}
                placeholder="e.g. 1779072345678-x8kfj2m"
                style={{
                  width: '100%',
                  padding: '10px 12px',
                  borderRadius: 8,
            ...
            <button
              onClick={runCompare}
              disabled={aiLoading || !compareOut.trim() || !compareIn.trim()}
              style={{
                ...btn(accent, '#fff'),
                opacity: aiLoading || !compareOut.trim() || !compareIn.trim() ? 0.4 : 1,
              }}
            >
            ...
            {aiResult && (
              <div style={{ marginTop: 20 }}>
                  ...
                  style={{
                    background:
                      aiResult.verdict === 'CLEAR'
                        ? '#eaf7f0'
                        : aiResult.verdict === 'MINOR' || aiResult.verdict === 'FEES_APPLY'
                          ? '#fef3c7'
                          : '#fcebeb',
                    border: `1.5px solid ${
                      aiResult.verdict === 'CLEAR'
                ...
                >
                  ...
                  >
                    {aiResult.verdict === 'CLEAR'
                      ? 'All clear — no damage detected'
                      : aiResult.verdict === 'MINOR'
                        ? 'Minor wear — review recommended'
                        : aiResult.verdict === 'FEES_APPLY'
                          ? 'Fees apply — see details'
                          : aiResult.verdict === 'CRITICAL'
                            ? 'Critical damage detected'
                            : 'Damage found — review required'}
                  </div>
                  <div
                    style={{ fontSize: 12, color: muted, marginBottom: aiResult.summary ? 8 : 0 }}
                  >
                ...
                >
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <div
                      style={{ background: bg, borderRadius: 8, padding: 10, textAlign: 'center' }}
                    >
                    ...
                    {aiResult.findings.map((f, i) => (
                      <div
                        ...
                        style={{
                          padding: '10px 0',
                          borderBottom:
                            i < aiResult.findings.length - 1 ? `1px solid ${bdr}` : 'none',
                          display: 'flex',
                          gap: 10,
                        }}
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div
                            style={{ fontSize: 12, color: muted, marginTop: 1, lineHeight: 1.4 }}
                          >
                            {f.note}
                          </div>
                          {f.confidence && (
                ...
                {aiResult.tokensUsed && (
                  <div style={{ marginTop: 12, fontSize: 10, color: muted, textAlign: 'center' }}>
                    Tokens: {aiResult.tokensUsed.input || 0} in / {aiResult.tokensUsed.output || 0}{' '}
                    out
                  </div>
                )}
              </div>
        ...
        {role && !compareMode && step === 'machine' && (
          <div style={{ marginTop: 20 }}>
            ...
            <div style={{ fontSize: 13, color: muted, marginBottom: 16 }}>
              {role === 'customer'
                ? 'Photograph the watercraft before your rental begins.'
                : 'Inspect the watercraft upon return.'}
            </div>
            ...
            {MACHINES.map((m) => (
              <button
                ...
                style={{
                  ...
                  alignItems: 'center',
                  textAlign: 'left',
                  opacity: customer.trim() ? 1 : 0.4,
                  cursor: customer.trim() ? 'pointer' : 'not-allowed',
                }}
              >
                <div>
            ...
            {role === 'customer' && (
              ...
              >
                <strong>Why am I doing this?</strong> These photos protect you. If there's
                pre-existing damage, your photos prove it was there before your rental. Take clear,
                well-lit photos of every zone.
              </div>
            )}
          </div>
        )}
        ...
        {role && !compareMode && step === 'inspect' && cz && (
          <div style={{ marginTop: 12 }}>
            <button
              onClick={() => (zone > 0 ? setZone(zone - 1) : setStep('machine'))}
              style={{ ...outline, padding: '6px 12px', fontSize: 12, marginBottom: 12 }}
            >
              ← Back
            </button>
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 11, color: muted }}>
                  {role === 'customer' ? 'Check-out' : 'Check-in'} · {customer}
                </div>
              </div>
              <div
                style={{
              ...
              {zones.map((z, i) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 2,
                    cursor: 'pointer',
                    background: i === zone ? accent : (zp[z.id] || []).length > 0 ? green : bdr,
                  }}
                />
              ))}
            </div>
            ...
            >
              ...
              {cp.length > 0 && (
                ...
                >
                  {cp.map((src, i) => (
                    <Thumb key={i} src={src} onRemove={() => rmPhoto(i)} />
                  ))}
                </div>
              )}
              ...
              <div style={{ marginTop: 10 }}>
                <input
                  ...
                  placeholder={
                    cz.id === 'impeller'
                      ? 'Note any nicks, bent blades, debris damage…'
            ...
            <div
              style={{
                display: 'grid',
                gridTemplateColumns:
                  zone === zones.length - 1 && done >= zones.length ? '1fr 1fr' : '1fr',
                gap: 8,
              }}
            >
              <button
                onClick={() => (zone < zones.length - 1 ? setZone(zone + 1) : null)}
                disabled={cp.length === 0 || zone === zones.length - 1}
                style={{
                  ...btn(cp.length > 0 && zone < zones.length - 1 ? accent : '#ccc', '#fff'),
                  cursor: cp.length > 0 && zone < zones.length - 1 ? 'pointer' : 'not-allowed',
                }}
              >
                {zone < zones.length - 1 ? 'Next zone →' : 'All zones complete'}
              </button>
              {zone === zones.length - 1 && done >= zones.length && (
                <button onClick={() => setStep('review')} style={btn(dark, '#fff')}>
                  Review & submit
                </button>
              )}
            </div>
        ...
        {role && !compareMode && step === 'review' && (
          <div style={{ marginTop: 12 }}>
            ...
            <div style={{ fontSize: 12, color: muted, marginBottom: 14 }}>
              {role === 'customer' ? 'Check-out' : 'Check-in'} · {customer} · {ts()}
            </div>
                ...
                return (
                  ...
                  >
                    <div style={{ fontSize: 13 }}>
                      ...
                      {znn && (
                        <span
                          style={{ color: isSpecial ? red : accent, fontSize: 11, marginLeft: 6 }}
                        >
                          {isSpecial ? '🚨' : '⚠'} {znn}
                        </span>
                      )}
                    ...
                    <span
                      style={{ fontSize: 12, fontWeight: 600, color: zpp.length > 0 ? green : red }}
                    >
                      {zpp.length} 📷
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    <button
                      ...
                      style={{
                        ...outline,
                        textAlign: 'center',
                        borderColor: fuelOk === true ? green : bdr,
                    ...
                    <button
                      ...
                      style={{
                        ...outline,
                        textAlign: 'center',
                        borderColor: fuelOk === false ? accent : bdr,
            ...
            <button
              ...
              disabled={uploading}
              style={{ ...btn(accent, '#fff'), opacity: uploading ? 0.5 : 1 }}
            >
              {uploading
                ? '⏳ Uploading...'
                : `✓ Submit ${role === 'customer' ? 'check-out' : 'check-in'}`}
            </button>
          </div>
        )}
        ...
        {step === 'done' && (
          <div style={{ marginTop: 40, textAlign: 'center' }}>
            ...
            {submitted.length > 0 && (
              <>
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      navigator.clipboard?.writeText(submitted[submitted.length - 1].id)
                    }
                    style={{ ...outline, padding: '5px 12px', fontSize: 11 }}
                  >
                    📋 Copy ID
                  </button>
                </div>
                ...
                >
                  ✅ Saved to inspection log.{' '}
                  {role === 'owner' ? 'Owner email + SMS sent.' : 'Owner has been notified.'}
                </div>
              </>
            )}
            ...
            {role === 'customer' && (
              ...
              >
                <strong>Your photos protect you.</strong> They're saved with your inspection ID
                above. If there's any damage dispute, this ID links to your photos.
              </div>
            )}
            ...
            >
              ...
              <button onClick={reset} style={{ ...outline, width: '100%' }}>
                Done
              </button>
            </div>
          </div>
