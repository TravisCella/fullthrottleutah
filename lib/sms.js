...
import { formatPickupAddressLine, PICKUP_MAPS_URL } from './business';
...
export function buildBookingConfirmationSMS(booking) {
  ...
  if (booking.white_glove) {
    ...
    lines.push(
      `Delivery: ${booking.pickup_time_display || '8:00 AM'} · Retrieval: ${booking.return_time_display || '8:00 PM'}`
    );
    lines.push(`We'll be in touch to confirm details.`);
  } else {
    lines.push(`Pickup: ${booking.pickup_time_display || '8:00 AM'}`);
    lines.push(formatPickupAddressLine());
    lines.push(`Map: ${PICKUP_MAPS_URL}`);
  ...
  lines.push(
    `At pickup: $${getDepositAmount(booking.package).toLocaleString()} security deposit (card hold or cash)`
  );
  lines.push(``);
