...
export function buildBookingConfirmationSMS(booking) {
  const due = Number(booking.total_price) - Number(booking.deposit_paid) + 1000;
  return [
    ...
    `Due at pickup: $${due} (balance + $1,000 deposit)`,
    ``,
    `Bring: driver's license, 2" ball hitch, 4-prong light hookup.`,
    ``,
    `Questions? Reply to this text or call 801-548-1273.`,
  ].join('\n');
}
