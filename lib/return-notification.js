...
function buildReturnEmailHTML({
  firstName, action, capturedAmount, releasedAmount, damageReason,
  packageName, location, startDate, endDate,
}) {
  const holdTotal = capturedAmount + releasedAmount;
  const isClean = action === 'release';

  const title = isClean
    ? 'Rental Complete — Hold Released'
    : 'Rental Complete — Deposit Summary';
  ...
  const datesLine = startDate && endDate && endDate !== startDate
    ? `${startDate} → ${endDate}`
    : startDate || '';
  ...
  const reasonRow = !isClean && damageReason
...
export function fireReturnNotification(depositHold, origPiId, { action, capturedAmount, releasedAmount, damageReason }) {
  _sendReturnNotification(depositHold, origPiId, { action, capturedAmount, releasedAmount, damageReason })
