...
export async function createBookingEvent(booking) {
  ...
  const dueAtPickup = Number(booking.total_price) - Number(booking.deposit_paid) + 1000;
  ...
  const event = {
    ...
    description: [
      ...
      `  (Remaining balance + $1,000 security deposit)`,
      ``,
      ...
      `• Collect remaining balance + $1,000 security deposit`,
