...
function parseRentalDate(dateStr) {
  ...
  const today = new Date(year, now.getMonth(), now.getDate()); // midnight today, local time
  const thisYear = new Date(`${cleaned}, ${year}`);
  // Bookings are always paid before the rental date. If this year's date has
  // already passed, the rental must be next year.
  const result = thisYear >= today ? thisYear : new Date(`${cleaned}, ${year + 1}`);
  if (Number.isNaN(result.getTime())) {
    console.warn(`[calendar] parseRentalDate: unparseable date "${dateStr}" — using today as fallback`);
    return new Date();
  }
  return result;
}
...
export async function createBookingEvent(booking) {
  ...
  const event = {
    ...
    description: [
      ...
      ``,
      `PICKUP: ${booking.pickup_time_display || '8:00 AM'} from Farmington, UT`,
      `RETURN: ${booking.return_time_display || '8:00 PM'}`,
      ``,
      `REMINDERS:`,
