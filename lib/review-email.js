...
export async function sendReviewRequest(paymentIntentId) {
  ...
  if (!bookingId) {
    try {
      ...
      if (bookingId) console.log(`[review-email] Resolved bookingId via session lookup: ${bookingId}`);
    } catch (lookupErr) {
      console.warn('[review-email] Session lookup failed for PI', paymentIntentId, ':', lookupErr.message);
    }
  }
