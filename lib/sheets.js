  ...
  try {
    ...
    if (blockRows.length > 1) {
      for (let i = 1; i < blockRows.length; i++) {
        ...
        const startDate = row[0]; // column A
        const endDate = row[1];   // column B
        const blockPkg = (row[2] || 'ALL').trim(); // column C
        ...
        if (packageId && blockPkg.toUpperCase() !== 'ALL') {
          if (!blockPkg.toLowerCase().includes(packageId.toLowerCase())) continue;
        }
