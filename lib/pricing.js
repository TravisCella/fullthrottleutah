...
// ─── Multi-day per-day discount schedule ──────────────────────────────────────
// Each rental day is discounted INDEPENDENTLY off the standard weekday daily
// rate; the base price is the SUM of every day's discounted rate — NOT one flat
// percentage applied to the whole booking. Day 1 is full price. The per-day
// discount is capped at 35% for day 4 and beyond (never exceeds MAX).
export const MULTI_DAY_DISCOUNTS = [0, 0.1, 0.3, 0.35]; // index = (day − 1)
export const MAX_MULTI_DAY_DISCOUNT = 0.35;

// dayIndex is 1-based. Day 4 and beyond all use the final (max) tier, so the
// per-day discount never exceeds 35%.
export function dailyDiscountRate(dayIndex) {
  const i = Math.min(Math.max(dayIndex, 1), MULTI_DAY_DISCOUNTS.length) - 1;
  return MULTI_DAY_DISCOUNTS[i];
}
...
export function calculateBasePrice(pkg, start, end) {
  ...
  // A single-day rental has no multi-day discount — keep the standard
  // weekday/weekend rate based on the start day.
  if (days === 1) return isWeekend(start) ? pkg.weekend : pkg.weekday;
  // Multi-day: sum each day's independently-discounted weekday rate. Round each
  // day to whole dollars, matching the codebase's Math.round convention.
  let total = 0;
  for (let d = 1; d <= days; d++) {
    total += Math.round(pkg.weekday * (1 - dailyDiscountRate(d)));
  }
  return total;
}
