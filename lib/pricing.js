...
export const PACKAGES = [
  {
    ...
    description: "A single, playful Trixx built for fun — extended hull for tow sports (wakeboard, tube, ski) and seating for up to 3. The low-commitment way to get one machine on the water without renting a pair.",
    includes: ["1 Sea-Doo Spark Trixx (3-up)", "Single trailer", "3 life preservers", "1 anchoring system", "Safety flags"],
    weekday: 239,
    weekend: 275,
...
export const LOYALTY_DISCOUNT_RATE = 0.10; // 10% off base price for returning customers
...
export function isWeekend(d) { const day = new Date(d).getDay(); return day === 0 || day === 5 || day === 6; }

// Verbatim from booking.js — accepts two Date objects.
export function daysBetween(a, b) { return Math.round((b - a) / 864e5) + 1; }
...
export function computeTotal({
  ...
  const loyaltyDiscount = repeatCustomer
    ? Math.round(basePrice * LOYALTY_DISCOUNT_RATE)
    : 0;

  const { adjustment: premiumAdjustment, reason: promoReason } =
    computePremiumAdjustment(premiums, start, end, basePrice);

  const total = Math.max(0,
    basePrice + holidaySurcharge + whiteGloveFee + deconFee + extraVestFee - loyaltyDiscount + premiumAdjustment
  );
